...
void ShaderForFFL_DrawCallback(void* pObj, const FFLDrawParam* pDrawParam)
{
    ...
    if (pDrawParam->modulateParam.pTexture2D != NULL)
    {
        ...
        GLuint textureHandle = FFL_GET_RIO_NATIVE_TEXTURE_HANDLE(pDrawParam->modulateParam.pTexture2D);
    ...
    if (pDrawParam->modulateParam.type < MATERIAL_PARAM_SIZE)
    {
...

{
    switch (boneIndex)
    {
        ...
        {
            scale->x = scaleFactors.x;
            float y = fmin(scaleFactors.y, 1.0f);
            scale->y = y;
            scale->z = scaleFactors.z;
            break;
        }
...
int main(void)
{
    ...
    */
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        Matrix headModelMatrix = MatrixMultiply(headBoneMatrix, matBodyScale);
            ...
            BeginMode3D(camera);
                ...
                */
                ...
                if (isFFLModelCreated) // draw only if it is valid
                {
...
#ifndef NO_MODELS_FOR_TEST
                    Matrix matModel = MatrixMultiply(MatrixScale(0.1, 0.1, 0.1), headModelMatrix);
#else
                    ...
                    UpdateCharModelBlink(&isBlinking, &lastBlinkTime, &charModel, now);
